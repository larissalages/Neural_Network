function [MSEtreino, MSEvalidacao, MSEteste, verdadeirosPositivos, verdadeirosNegativos, falsosPositivos, falsosNegativos, AUC] = mlp(fileTreino, fileValidacao, fileTeste, amostrasTreino, amostrasValidacao, amostrasTeste, neuroniosEscondidos, taxaAprendizagem, funcaoAtivacao, algoritmoAprendizagem)

%   Informacoes sobre a rede e os dados
numEntradas   = 46;     % Numero de nodos de entrada
numSaidas     = 2;     % Numero de nodos de saida
numEscondidos = neuroniosEscondidos;    % Numero de nodos escondidos
numTr         = amostrasTreino;   % Numero de padroes de treinamento
numVal        = amostrasValidacao;    % Numero de padroes de validacao
numTeste      = amostrasTeste;    % Numero de padroes de teste
taxaAprendizado = taxaAprendizagem;

%    Abrindo arquivos 
arquivoTreinamento = fopen(fileTreino,'rt');  
arquivoValidacao   = fopen(fileValidacao,'rt');    
arquivoTeste       = fopen(fileTeste,'rt');        
...
rede = newff(matrizFaixa,[numEscondidos numSaidas],{funcaoAtivacao, 'tansig'}, algoritmoAprendizagem, 'learngdm', 'mse');
...
% 'mse'                          : indica que o erro a ser utilizado vai ser MSE (soma dos erros quadraticos)
...
rede.trainParam.lr       = taxaAprendizado;  % Taxa de aprendizado
...
MSEtreino = desempenho.perf(length(desempenho.perf));
MSEvalidacao = desempenho.vperf(length(desempenho.vperf));
MSEteste = desempenhoTeste;

%ROC
%Pega segunda linha das matrizes, a classe 1 eh a que tem a segunda linha
%com o valor 1
plotroc(saidasTeste,saidasRedeTeste);
labels =saidasTeste(2,:);
scores = saidasRedeTeste(2,:);
posclass = 1;
...
for padrao = 1 : numTeste; 
    %verdadeiro positivo
    if (nodoVencedorRede(padrao) == 2 && nodoVencedorDesejado(padrao) == 2)
        verdadeirosPositivos = verdadeirosPositivos + 1;
    %verdadeiro negativo
    elseif (nodoVencedorRede(padrao) == 1 && nodoVencedorDesejado(padrao) == 1)
        verdadeirosNegativos = verdadeirosNegativos + 1;
    %falso positivo
    elseif (nodoVencedorRede(padrao) == 1 && nodoVencedorDesejado(padrao) == 2)
        falsosNegativos = falsosNegativos + 1;
    %falso negativo
    elseif (nodoVencedorRede(padrao) == 2 && nodoVencedorDesejado(padrao) == 1)
